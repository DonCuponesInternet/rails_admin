...
module RailsAdmin
  module ApplicationHelper
    ...
    def edit_user_link
      ...
      link_to url_for(action: edit_action.action_name, model_name: abstract_model.to_param, id: _current_user.id, controller: 'rails_admin/main'), style: "#{"color: #f7f7f7 !important" if I18n.locale == :milenio}" do
        html = []
    ...
    def menu_for(parent, abstract_model = nil, object = nil, only_icon = false) # perf matters here (no action view trickery)
      ...
      actions.reject!{|action|
        ...
        if action.class == RailsAdmin::Config::Actions::Edit
          ...
          !included
        elsif (action.class == RailsAdmin::Config::Actions::Top100Stores || action.class == RailsAdmin::Config::Actions::Latest25Stores)
          (abstract_model.model_name.constantize != Store) rescue true
          ...
          true
        elsif action.class == RailsAdmin::Config::Actions::ByActivity
          (abstract_model.model_name.constantize != Store) rescue true
        else
          false
        end
    ...
    def bulk_menu(abstract_model = @abstract_model)
      ...
      content_tag :li, class: 'dropdown', style: 'float:right' do
        content_tag(:a, class: 'dropdown-toggle bulk-menu-toggler', data: {toggle: 'dropdown'}, href: '#') { t('admin.misc.bulk_menu_title').html_safe + ' ' + '<b class="caret"></b>'.html_safe } +
          content_tag(:ul, class: 'dropdown-menu', style: 'left:auto; right:0;') do
            actions.collect do |action|
              content_tag :li do
                link_to(wording_for(:bulk_link, action), '#', class: "bulk-action-trigger-#{action.action_name}", onclick: "jQuery('#bulk_action').val('#{action.action_name}'); jQuery('#bulk_form').submit(); return false;")
              end
            end.join.html_safe
          end
      end.html_safe
